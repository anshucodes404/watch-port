                 ...
                 match port.pid {
                     Some(pid) => {
                         ...
                         self.last_action_msg = Some(match res {
                             Ok(_) => format!(
                                 " Sent SIGTERM to {} (pid{})", port.name.as_deref().unwrap_or("?"), pid
                             ),
                             Err(e) => format!(" Kill failed: {e}"),
                         });
                     }
                     None => {
                         self.last_action_msg = Some(" Cannot kill: PID unknown (try running with sudo)".into());
                     }
                 }
             }
         }
